...
$example_name = 'Register affiliate for a product';
...
if(!empty($_POST['affiliate_id'])) {
	...
	try {
		$response = $tc->registerAffiliate($affiliate_id, array(
		...
		echo '<div class="notification is-danger is-light">There was an error while registering this affiliate: '.$e->getMessage().'</div>';
	}
}
...
if(empty($_POST['affiliate_id'])) {
	echo '<article class="message is-info"><div class="message-body">';
		echo 'In this example, we will register an existing affiliate for a product. We can choose whether to auto-approve them regardless of the product settings.';
	echo '</div></article>';

	echo '<form action="example_affiliate_register.php?access_token='.$access_token.'" method="post">';
		...
		echo '<div class="field">';
			echo '<label class="label">Enter the numeric product ID to add them to</label>';
			echo '<div class="control">';
		...
		echo '<div class="field is-grouped">';
			echo '<div class="control">';
				echo '<button class="button is-link">Register affiliate for product</button>';
			echo '</div>';
		echo '</div>';
