...
$example_name = 'Reject an affiliate application';
...
if(!empty($_POST['affiliate_id'])) {
	...
	try {
		$response = $tc->rejectAffiliate($affiliate_id, array(
		...
		echo '<div class="notification is-danger is-light">There was an error while rejecting this affiliate: '.$e->getMessage().'</div>';
	}
}
...
if(empty($_POST['affiliate_id'])) {
	echo '<article class="message is-info"><div class="message-body">';
		echo 'In this example, we will reject an affiliate\'s pending application, and/or revoke their access to promote a product.';
	echo '</div></article>';

	echo '<form action="example_affiliate_reject.php?access_token='.$access_token.'" method="post">';
		...
		echo '<div class="field">';
			echo '<label class="label">Enter the numeric product ID to reject them for</label>';
			echo '<div class="control">';
		...
		echo '<div class="field is-grouped">';
			echo '<div class="control">';
				echo '<button class="button is-link">Reject or revoke affiliate</button>';
			echo '</div>';
		echo '</div>';
