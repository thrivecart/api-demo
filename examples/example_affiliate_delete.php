...
$example_name = 'Delete an affiliate from your account';
...
if(!empty($_POST['affiliate_id'])) {
	...
	try {
		$response = $tc->deleteAffiliate($affiliate_id);
		...
		echo '<div class="notification is-danger is-light">There was an error while deleting this affiliate: '.$e->getMessage().'</div>';
	}
}
...
if(empty($_POST['affiliate_id'])) {
	echo '<article class="message is-info"><div class="message-body">';
		echo 'In this example, we will delete an affiliate from your account entirely. Note that their login information will still work, as affiliate accounts exist globally to the ThriveCart platform, but they will have no further access to your account.';
	echo '</div></article>';

	echo '<form action="example_affiliate_delete.php?access_token='.$access_token.'" method="post">';
		...
		echo '<div class="field is-grouped">';
			echo '<div class="control">';
				echo '<button class="button is-link">Delete affiliate</button>';
			echo '</div>';
		echo '</div>';
