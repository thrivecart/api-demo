...
$example_name = 'Favorite affiliate';
...
if(!empty($_POST['affiliate_id'])) {
	...
	try {
		$response = $tc->favoriteAffiliate($affiliate_id);
		...
		echo '<div class="notification is-danger is-light">There was an error while marking this affiliate as a favorite: '.$e->getMessage().'</div>';
	}
}
...
if(empty($_POST['affiliate_id'])) {
	echo '<article class="message is-info"><div class="message-body">';
		echo 'In this example, we will mark a particular affiliate as a favorite/VIP. This is displayed inside the ThriveCart dashboard, and you can easily search down to just your VIP affiliates.';
	echo '</div></article>';

	echo '<form action="example_affiliate_favorite.php?access_token='.$access_token.'" method="post">';
		...
		echo '<div class="field is-grouped">';
			echo '<div class="control">';
				echo '<button class="button is-link">Mark as favorite</button>';
			echo '</div>';
		echo '</div>';
